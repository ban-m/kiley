...
fn main() {
    ...
    let result: Vec<_> = parameters
        ...
        .collect();
    println!("ErrorRate\tRadius\tBandedTime\tBandedDist\tGuidedTime\tGuidedDist\tBefore\tSeed");
    for (er, rad, result) in result.iter() {
        for (btime, bdist, gtime, gdist, before, s) in result {
            println!(
                "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}",
                er, rad, btime, bdist, gtime, gdist, before, s
            );
        }
    }
}
type BenchResult = (f64, usize, Vec<(u128, u32, u128, u32, u32, u64)>);
fn run_bench(len: usize, coverage: usize, error_rate: f64, rad: usize, seed: u64) -> BenchResult {
    ...
    let times: Vec<_> = (0..50)
        ...
        .map(|i| {
            ...
            (btime, bdist, gtime, gdist, before, seed + i)
        })
        .collect();
